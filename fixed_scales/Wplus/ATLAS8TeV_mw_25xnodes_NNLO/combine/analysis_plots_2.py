...
def plot_combined(
    ...
    for col, order in enumerate(orders):
        ...
        if order == "NLO":
            ...
            kenv = kfactor_envelope(
                df_store["NLO"], df_store["LO"],
                num_col="NLO", den_col="LO",
                central_scale=central_scale_factor,
            )
            plot_kfactor_panel(ax_kfactor, kenv,
                               color="tab:purple", label="kNLO")
            ...
            kenv = kfactor_envelope(
                df_store["NNLO"], df_store["NLO"],
                num_col="NNLO", den_col="NLO",
                central_scale=central_scale_factor,
            )
            plot_kfactor_panel(ax_kfactor, kenv,
                               color="tab:red", label="kNNLO")
            ax_kfactor.set_xlabel(X_LABEL[observable])
